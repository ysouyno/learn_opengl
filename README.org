...
2. 因为希望在摄像机的坐标系中跟局部坐标系保持一致，也是屏幕指向我的方向是正方向，所以上面代码中使用了 ~(cameraPos - cameraTarget)~ 来反方向 ~z~ 的方向。
...
~GL_RGB~ 颜色值被限制在了 ~0.0~ 到 ~1.0~ 之间，当一个帧缓冲的颜色缓冲的内部格式被设定成了 ~GL_RGB16F~ ， ~GL_RGBA16F~ ， ~GL_RGB32F~ 或者 ~GL_RGBA32F~ 时，这些帧缓冲被叫做浮点帧缓冲（ ~Floating Point Framebuffer~ ），浮点帧缓冲可以存储超过 ~0.0~ 到 ~1.0~ 范围的浮点值，所以非常适合 ~HDR~ 渲染。

* <2024-10-30 周三> 高级光照.泛光

书中并没有给出阶段代码，用 ~HDR~ 实现的泛光的原始场景，我在 ~advanced_lighting_hdr~ 的基础上实现了，效果如下：

#+ATTR_HTML: :width 60%
[[file:files/20241030_0.png]]

我现在才算真正理解帧缓冲的整个流程：

1. 帧缓冲对象 ~hdrFBO~ 的颜色缓冲 ~colorBuffer~ 是最终从 ~0~ 渲染至 ~Framebuffer~ 的媒介。
2. 在主循环中绘制时，所有操作都渲染至帧缓冲中。
3. 然后调用 ~glBindFramebuffer(GL_FRAMEBUFFER, 0);~ 将接下来的操作渲染至默认环境中。
4. 然后将帧缓冲的颜色缓冲 ~colorBuffer~ 作为片段着色器的纹理渲染至默认环境中。
5. ~renderQuad~ 函数其实就是将整个纹理全渲染至默认环境中。

** 提取亮色

有必要对当前代码做一下解释：

+ 当前有两个帧缓冲，分别是 ~GL_COLOR_ATTACHMENT0~ 和 ~GL_COLOR_ATTACHMENT1~ ，前者是正常显示，后者是提取亮色。
+ 在调用 ~renderQuad();~ 之前的代码 ~glBindTexture(GL_TEXTURE_2D, colorBuffers[1]);~ 可以修改显示哪个帧缓冲
  - 使用 ~colorBuffers[0]~ 就是显示 ~GL_COLOR_ATTACHMENT0~ 。
  - 使用 ~colorBuffers[1]~ 就是显示 ~GL_COLOR_ATTACHMENT1~ 。
+ 即片段着色器中
  - 编号 ~location = 0~ 的 ~FragColor~ 输出的颜色对应于 ~GL_COLOR_ATTACHMENT0~ 的帧缓冲。
  - 编号 ~location = 1~ 的 ~BrightColor~ 输出的颜色对应于 ~GL_COLOR_ATTACHMENT1~ 的帧缓冲。

但是我的效果跟书中的有点不太一样，第一张是书中的图，第二张是我的图：

#+ATTR_HTML: :width 60%
[[file:files/bloom_attachments.png]]

#+ATTR_HTML: :width 60%
[[file:files/20241030_1.png]]

原来如此，我只处理了灯光的片段着色器而没有处理物体的片段着色器，所以才导致与书中的图效果不一样。经过修改，已达到和书中的效果一致：

#+ATTR_HTML: :width 60%
[[file:files/20241030_2.png]]

** 高斯模糊

怎么感觉我的模糊效果颠倒了，光源没有模糊，箱子却模糊了：

#+ATTR_HTML: :width 60%
[[file:files/20241030_3.png]]

箱子模糊的问题找到了，因为我的代码箱子的着色器是 ~shaderBlur~ ，它的输出变量 ~FragColor~ 进行了高斯模糊，所以箱子模糊了，而光源用的着色器是 ~shaderLight~ ，但是这里不能在 ~shaderLight~ 中处理高斯模糊，处理高斯模糊是在颜色缓冲中做的，这里有两个颜色缓冲，提取亮色的缓冲是 ~colorBuffers[1]~ ，这样流程就清晰了吧！

在 ~pingpongColorbuffers[!horizontal]~ 这个颜色缓冲中存放的就是高斯模糊的图像，我把它单独显示出来：

#+ATTR_HTML: :width 60%
[[file:files/20241031_0.png]]

** 把两个纹理混合

这里使用了新的着色器 ~shaderBloomFinal~ ，这里也没有对 ~FragColor~ 进行模糊处理，它设置了两个纹理：
...
分别对应 ~colorBuffers~ 和 ~pingpongColorbuffers~ ，代码中即：
...
#+ATTR_HTML: :width 60%
[[file:files/20241031_1.gif]]

* <2024-11-07 周四> 高级光照.延迟着色法（一）

** <2024-11-07 周四> ~G~ 缓冲（一）

从文章了解到 ~G~ 缓冲可以包括如下几个颜色缓冲：

#+ATTR_HTML: :width 60%
[[file:files/deferred_g_buffer.png]]

我想实现一下，所以思路是：

1. 首先要有两个 ~shader~ ：
   + 一个负责在 ~framebuffer~ 中生成上面的四个颜色缓冲。
   + 一个负责从 ~framebuffer~ 取颜色纹理渲染至窗口。
2. 使用多渲染目标（ ~Multiple Render Targets~ ，即 ~MRT~ ）技术。

我用的是第二个 ~shader~ 是 ~debug.vs~ 和 ~debug.fs~ ：
...
#+begin_src c++
  ...
  renderQuad();
  glViewport(0, 0, SCR_WIDTH, SCR_HEIGHT); // 注意这里 <---
#+end_src

注意 ~glViewport(0, 0, SCR_WIDTH, SCR_HEIGHT);~ 必须要添加上，如果没有它，将会显示：

#+ATTR_HTML: :width 60%
[[file:files/20241107_0.png]]

虽然效果不对，但是感觉就快接近成功了，目前效果：

#+ATTR_HTML: :width 60%
[[file:files/20241107_1.png]]

** <2024-11-10 周日> ~G~ 缓冲（二）

原来只需要在上节代码中把 ~debugShader~ 的 ~glActiveTexture(GL_TEXTURE1);~ 这行代码删除即可：

#+ATTR_HTML: :width 60%
[[file:files/20241110_0.png]]

解析如下：

1. ~deubgShader~ 的 ~GL_TEXTURE0~ 本就可以代表整个窗口的纹理。
2. 所有的绘制不能再绘制到第二个纹理中了，比如上节代码中的多增加的这个 ~GL_TEXTURE1~ 纹理就导致了没有效果。

对代码做了一些小小的优化，最终效果如下：

#+ATTR_HTML: :width 60%
[[file:files/20241110_1.png]]

** 延迟光照处理阶段

目前我还没有理解，在帧缓冲里就这么增加了几个颜色缓冲，就实现延迟光照了？

当前代码效果：

#+ATTR_HTML: :width 60%
[[file:files/20241110_2.png]]

* <2025-06-05 周四> 复习（三）

** 第一章，《你好，三角形》

可能现在我才真正理解 ~VAO~ 和 ~VBO~ 的意思：

+ ~VBO~ ：只是一段内存，有五个 ~float~ 值，但是它不知道前三个表示 ~xyz~ 坐标，后两个表示纹理 ~xy~ 坐标。
+ ~VAO~ ：知道这个 ~VBO~ 中前三个表示 ~xyz~ 坐标，后两个表示纹理 ~xy~ 坐标。即 ~VAO~ 中存储这些 ~VBO~ 的“顶点属性配置”。
...
见上面代码中的 ~// glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);~ ，所以基本上永远也不应该写上这行代码才对！
...
点乘和叉乘是对向量而言，在 ~opengl~ 上向量是表示 ~Nx1~ 矩阵，即 ~N~ 行 ~1~ 列矩阵。

1. 点乘：两个向量的点乘等于它们的数乘结果乘以两个向量之间夹角的余弦值。
2. 叉乘：叉乘只在 ~3D~ 空间中有定义，它需要两个不平行向量作为输入，生成一个正交于两个输入向量的第三个向量。

*** 为什么单独的数字要叫做标量 ~Scalar~ ？

因为不管向量还是矩阵与标量的相乘都相当于它的每一个元素分别乘以该标题，就相当于缩放了所有元素。

*** 矩阵缩放

矩阵缩放：矩阵叉乘向量， ~w~ 分量没有意义。矩阵是变换矩阵，效果是向量发生了缩放。

*** 矩阵位移

矩阵位移：矩阵叉乘向量， ~w~ 分量必不可少。矩阵是变换矩阵，效果是向量发生了位移。

1. 如果你用 ~3x3~ 矩阵我们的位移值就没地方放也没地方乘了，所以是不行的。
2. 齐次坐标允许我们在 ~3D~ 向量上进行位移（如果没有 ~w~ 分量我们是不能位移向量的）。
3. 如果一个向量的齐次坐标是 ~0~ ，这个坐标就是方向向量，因为 ~w~ 坐标是 ~0~，这个向量就不能位移。

* <2025-06-23 周一>  复习（五）

** 第四章，《深度测试》

*** 为什么要将片段的非线性的深度值变换至线性深度值？

#+begin_quote
在 ~OpenGL~ 中，深度缓冲存储的深度值是非线性的，这意味着深度值与观察空间的 ~z~ 值之间存在非线性关系。这种非线性关系是为了在透视投影中更好地模拟人眼的视觉效果，但这也导致在深度测试中难以直接比较和判断片段的深度顺序。因此，将非线性的深度值变换为线性的深度值可以更好地进行深度测试和渲染。
#+end_quote
